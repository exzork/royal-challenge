        ...
        <div className="w-full min-h-screen bg-primary flex justify-center">
          <div className="max-w-7xl">
            ...
            <div className="mt-12 grid grid-cols-1 md:grid-cols-3">
              <img id="imageSrc" className="" ref={imageCanvas} src="/images/ninym.png" onLoad={() => setImageLoaded(true)} />
              <div className="w-auto flex flex-col mx-10">
                <div className="flex w-full">
                  <div className="flex-1 text-xl text-white">Red</div>
                  <input className="flex-1" type="range" min="-100" max="100" value={RPercent} onChange={(e) => setRPercent(parseInt(e.target.value))} />
                </div>
                <div className="flex w-full">
                  <div className="flex-1 text-xl text-white">Green</div>
                  <input className="flex-1"  type="range" min="-100" max="100" value={GPercent} onChange={(e) => setGPercent(parseInt(e.target.value))} />
                </div>
                <div className="flex w-full">
                  <div className="flex-1 text-xl text-white">Blue</div>
                  <input className="flex-1"  type="range" min="-100" max="100" value={BPercent} onChange={(e) => setBPercent(parseInt(e.target.value))} />
                </div>
              </div>
              <canvas id="new"/>
            </div>
